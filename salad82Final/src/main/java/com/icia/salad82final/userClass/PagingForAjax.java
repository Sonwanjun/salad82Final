...
public class PagingForAjax {
	private int maxNum; 		// 전체 글의 개수
	private int pageNum; 		// 현재 페이지 번호
	private int listCount; //10		// 페이지당 나타낼 글의 갯수
	private int pageCount; //2		// 페이지그룹당 페이지 갯수
	private String infoName; 	// 게시판의 종류

	public PagingForAjax(int maxNum, int pageNum, int listCount, int pageCount, String infoName) {
		this.maxNum = maxNum;
	...
	public String makeHtmlPaging() {
		...
		return makeHtml(currentGroup, totalPage, infoName);
	}

	private String makeHtml(int currentGroup, int totalPage, String infoName) {
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { //현재 페이지가 아닌 경우 링크처리
				sb.append("<a href='javascript:"+infoName+"(" + i + ")'>");
				sb.append(" [ ");
				sb.append(i);
				sb.append(" ] ");
				sb.append("</a>");
